package com.lenily.dream.manager.content;
...
public class ExceptionNotice {
	...
	public ExceptionNotice(Throwable ex, String filterTrace, Object[] args) {
		...
		Stream<StackTraceElement> stream = Arrays.stream(ex.getStackTrace());
		...
		this.traceInfo = Arrays.stream(ex.getStackTrace()).map(x -> x.toString()).collect(toList());
		this.methodName = filterTrace;
		this.uid = calUid();
